 <?php
    ...
    //获取页码
      if(!empty($_GET['page']) && isset($_GET['page'])){
        ...
        $class = 'pagination';
        $offset = ($page-1)* $limit;
        $newsinfo = $db->select_all('lay_news','*'," as a INNER JOIN lay_news_type as b  ON a.news_type_id = b.news_type_id  LIMIT {$offset},{$limit}",'assoc');
        ...
        if(isset($_GET['del']) && !empty($_GET['del'])){
             $id=$_GET['del'];
             $newsfind = $db->find_all('lay_news','*'," WHERE `news_id`={$id}");
             $res = $db->del('lay_news'," WHERE `news_id`={$id}");
             if($res['code']==1){
              //删除本地图片
              $localimg = explode('/',$newsfind['news_img']);
			...
			<div class="panel">
                ...
                <div class="panel-body">
                  ...
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php  if($newsinfo != null){ foreach($newsinfo as $v){ ?>
                      <tr class="success">
                        <td class="text-center"><input type="checkbox" value="1" name="idarr[]" class="cbox"></td>
                        <td><?php echo $v['news_title'] ?></td>
                        <td ><?php echo $v['news_content'] ?></td>
                        <td width="200"><?php echo date('Y-m-d H:i:s',$v['news_time'])?></td>
                        <td><?php echo $v['news_type_name']?></td>
                        <td>
                          <div class="btn-group">
                            <a href="news_add.php?edit=<?php  echo $v['news_id'] ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
